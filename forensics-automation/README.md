...
The first part gets the domain names, the second part cuts the domain names and gets only the first part of it, i.e cleans up the .windowsliveupdater.com portion of each resolution, then "uniq" combines/removes non-unique consequtive lines which cleans up the output a little. This yields the following output:

<details>
  <summary> Long formatted output </summary>
...
From here, we can either copy the hexes into the program manually, or continue with trying to automate it. I found it useful to paste the hex lines with '\n' seperation into "tr -d '\n'" which would remove all newlines and provide a single long chain.
...
The full key is thus "HTB{y0u_c4n_4utom4t3_but_y0u_c4nt_h1de}". Challenge complete!
