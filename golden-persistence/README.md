...
The following line in particular will execute the encoded command, which in this case would be the final payload of this script.
```powershell
$DecryptedString|iex
```

The command is recoeverd from several other registry entries, which can be seen in the bottom portion of the script.
...
Manually obtaining these strings and plugging them into the script, we can recover what the command actually is. I also switched around the encoding statement for $enc, since I did not get the script to execute otherwise. 
